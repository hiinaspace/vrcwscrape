...
# Table-1 metric keys that must always be present in the slimmed summary.
TABLE1_SUMMARY_KEYS = (
    "parcel_tri_count",
    "parcel_quad_count",
    "parcel_pent_count",
...
class GeneratedStub:
    ...
    partition_lines: list[tuple[int, LineString]] | None = None

    def __post_init__(self) -> None:
        # Default partition_lines to tagged split_lines if not provided.
        if self.partition_lines is None:
            object.__setattr__(
                self,
                "partition_lines",
                [(1, line) for line in self.split_lines],
            )


def _generated_stub(name: str = "square") -> Any:
...
def test_write_strict_chen_artifacts_writes_visual_geojson_and_manifest(
    ...
    for filename in ARTIFACT_FILES.values():
        assert (tmp_path / filename).exists(), f"missing artifact file: {filename}"
    ...
    # Table-1 keys must be present in slimmed summary.
    for key in TABLE1_SUMMARY_KEYS:
        assert key in manifest["summary"], f"missing Table-1 key: {key}"

    # Optimization regularity keys present.
    for key in OPTIMIZATION_REGULARITY_KEYS:
        assert key in manifest["summary"], f"missing regularity key: {key}"
    ...
    # Old heavy diagnostic keys must NOT be present (slimmed out).
    for key in manifest["summary"]:
        assert not key.startswith("chen_fig7_"), (
            f"chen_fig7_ key survived slimming: {key}"
        )
        assert not key.startswith("accepted_streamline_score_"), (
            f"accepted_streamline_score_ key survived slimming: {key}"
        )
        assert not key.startswith("accepted_streamline_field_b_"), (
            f"accepted_streamline_field_b_ key survived slimming: {key}"
        )

    assert manifest["summary"]["generation_stage"] == "unknown"
    assert manifest["summary"]["streamline_config_mode"] == "unknown"
    assert manifest["summary"]["optimization_stage"] == "unknown"
    assert manifest["files"]["svg"] == "chen_strict_layout.svg"
    assert "partition_lines" in manifest["files"]
    ...
    partition_lines = json.loads((tmp_path / "partition_lines.geojson").read_text())
    ...
    # Partition-lines layer: one line with level tag
    assert len(partition_lines["features"]) == 1
    assert partition_lines["features"][0]["properties"]["level"] == 1
...
def test_oval_artifacts_preserve_curved_split_and_street_polylines(
    ...
    # Table-1 keys present for a real run.
    for key in TABLE1_SUMMARY_KEYS:
        assert key in manifest["summary"], f"missing Table-1 key: {key}"
    ...
    # Partition-lines layer present.
    partition_lines = json.loads((tmp_path / "partition_lines.geojson").read_text())
    assert len(partition_lines["features"]) > 0
    for feat in partition_lines["features"]:
    ...
    # compare_to_paper present for oval -> ellipse row.
    compare = manifest["summary"]["compare_to_paper"]
    assert compare is not None
    assert "quad_fraction" in compare
    assert "irregularity_avg" in compare
    assert "junction_angle_dev_from_90_avg" in compare
    assert "streets_per_parcel" in compare
...
def test_strict_shape_runner_uses_expected_generation_interface(
    ...
    setattr(fake_module, "generate_named_layout", generate_named_layout)
...
def test_shapes_runner_accepts_min_parcel_area_flag(
    tmp_path: Path, monkeypatch: Any
) -> None:
    """--min-parcel-area bypasses the count-based mapping."""
    calls: list[dict] = []
    ...
    def generate_layout_for_boundary(boundary, *, min_parcel_area, seed, **kwargs):  # type: ignore[no-untyped-def]
        calls.append({"min_parcel_area": min_parcel_area, "boundary": boundary.name})
        return _generated_stub(boundary.name)

    def boundary_preset(name: str, width: float = 180.0, height: float = 140.0):  # type: ignore[no-untyped-def]
        return _FakeBoundary(name)

    fake_module = types.ModuleType("mapgen.chen_generate")
    setattr(fake_module, "generate_layout_for_boundary", generate_layout_for_boundary)
    setattr(fake_module, "boundary_preset", boundary_preset)
    monkeypatch.setitem(sys.modules, "mapgen.chen_generate", fake_module)

    runner = _load_runner_module()
    runner.run_shapes(
        out_dir=tmp_path,
        min_parcel_area=500.0,
        seed=0,
        width=180.0,
        height=140.0,
    )

    assert all(c["min_parcel_area"] == 500.0 for c in calls)
    assert len(calls) == 3
