...
| **S7b** | road-width model + per-fronting-tier setback + near-highway displacement. Shared `road_width_units(edge)` from `MacroEdge.betweenness` (+ tier floor) in `r1_mesh`; each lot's front setback denominated from the tier/width of the road its frontage fronts (highway → widest); lots within a highway's half-width+clearance EXCLUDED from assignment so the Hungarian re-displaces those worlds (reuse slice-P `assign_worlds_excluding`) — the real buildings-off-arterials fix (wave2 slice-1 used a flat `road_clear_m` placeholder). | `r1_mesh.py`, `r1_lots.py`, `run_r1_hybrid.py` | render (buildings clear of wide roads) + displacement stats |
